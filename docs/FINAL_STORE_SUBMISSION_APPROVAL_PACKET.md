...
| Missing provider configuration / expected fail-closed behavior | Contributions/add-on provider initialization returns HTTP 503 without verified Stripe credentials; RevenueCat offerings, restore, and webhook processing return HTTP 503 when unconfigured. Recurring web subscriptions use active RevenueCat Billing, not the retired direct-Stripe endpoint. That legacy endpoint alone returns HTTP 410 `subscription_checkout_migrating` to prevent a second recurring billing rail. Tests assert these provider boundaries instead of expecting synthetic purchase success. |
