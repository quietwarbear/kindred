...
async def steward_briefing(current_user: dict[str, Any] = Depends(get_current_user)):
    ...
    members = await users_collection.find(
        {"community_id": community_id}, {"_id": 0, "id": 1, "full_name": 1, "created_at": 1}
    ).to_list(1000)
    ...
    if oldest_mem:
        ...
        oldest_thr = await threads_collection.find(
            {"community_id": community_id, "title": {"$nin": ["", None]}}, {"_id": 0, "id": 1, "title": 1}
        ).sort("created_at", 1).to_list(1)
        if oldest_thr:
            rediscover = {"type": "thread", "id": oldest_thr[0]["id"], "title": oldest_thr[0].get("title", "")}
    ...
    notes = await generate_steward_notes(api_key, model, {
    ...
    return {
        ...
        "welcome": (
            {"member_id": newest["id"], "member_name": newest.get("full_name", ""), "message": notes["welcome_message"]}
            if newest and notes["welcome_message"] else None
        ),
        "quiet_members": quiet,
        "rediscover": ({**rediscover, "note": notes["rediscover_note"]} if rediscover else None),
