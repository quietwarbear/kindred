...
async def community_health(current_user: dict[str, Any] = Depends(get_current_user)):
    ...
    members = await users_collection.find(
        ...
        {"_id": 0, "id": 1, "full_name": 1, "role": 1, "created_at": 1},
    ).to_list(2000)
    ...
    threads = await threads_collection.find(
        {"community_id": community_id},
        {"_id": 0, "created_by": 1, "created_at": 1, "category": 1, "elder_name": 1},
    ).to_list(3000)
    ...
    participation_rate = (
        round(100 * len(active) / members_total) if members_total else 0
    )

    paid = await payments_collection.find(
        {"community_id": community_id, "payment_status": "paid"},
        {"_id": 0, "amount": 1, "user_id": 1, "user_email": 1},
    ).to_list(3000)
    funds_raised = round(sum(p.get("amount", 0) or 0 for p in paid), 2)
    financial_contributors = len(
        {
            (p.get("user_id") or p.get("user_email"))
            for p in paid
            if (p.get("user_id") or p.get("user_email"))
        }
    )
    ...
    youth_reflections = len(
        [t for t in threads if t.get("category") == "youth-reflection"]
    )
    ...
    return {
        ...
        "intergenerational": {
            "elder_voices": elder_voices,
            "youth_reflections": youth_reflections,
        },
        "archive": {
            "gatherings": len(events),
