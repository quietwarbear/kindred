...
async def get_community_modules(
    ...
    return {
        "enabled": resolve_modules(community),
...
async def set_community_modules(
...
async def get_overview(current_user: dict[str, Any] = Depends(get_current_user)):
    ...
    thread_count = await threads_collection.count_documents(
        {"community_id": community_id}
    )
    pending_invites = await invites_collection.find(
        {"community_id": community_id, "status": "pending"}, {"_id": 0}
    ).to_list(20)
    ...
    recent_threads = (
        await threads_collection.find({"community_id": community_id}, {"_id": 0})
        .sort("created_at", -1)
        .to_list(6)
    )
    ...
    return {
        ...
        "upcoming_events": [
            serialize_event_for_user(event, current_user) for event in upcoming_events
        ],
...
async def courtyard_home(current_user: dict[str, Any] = Depends(get_current_user)):
    ...
    community_id = current_user["community_id"]
    members = await users_collection.find(
        {"community_id": community_id}, {"_id": 0, "password_hash": 0}
    ).to_list(500)
    subyards = (
        await subyards_collection.find({"community_id": community_id}, {"_id": 0})
        .sort("created_at", 1)
        .to_list(50)
    )
    upcoming_events = (
    ...
    budgets = (
        await budget_plans_collection.find(
    ...
    kinships = (
        await kinships_collection.find({"community_id": community_id}, {"_id": 0})
        .sort("created_at", -1)
        .to_list(50)
    )
    announcements = (
        await announcements_collection.find({"community_id": community_id}, {"_id": 0})
        .sort("created_at", -1)
        .to_list(10)
    )
    ...
    return {
        ...
        "quick_actions": [
            {
                "id": "plan-gathering",
                "label": "Plan New Gathering",
                "target": "/gatherings",
            },
            {
                "id": "upload-story",
                "label": "Upload Photos/Stories",
                "target": "/timeline",
            },
            {
                "id": "check-funds",
                "label": "Check Shared Funds",
                "target": "/funds-travel",
            },
        ],
        "notifications": notifications,
        "invite_reminders": invite_reminders,
        "relationship_groups": sorted(
            {kinship["relationship_type"] for kinship in kinships}
        )[:10],
        "recent_timeline": [
            *memories[:3],
            *threads[:3],
        ...
        "role_catalog": [
            {"role": role, "tools": tools} for role, tools in ROLE_TOOLING.items()
        ],
    }
...
async def courtyard_structure(current_user: dict[str, Any] = Depends(get_current_user)):
    ...
    invites = (
        await invites_collection.find({"community_id": community_id}, {"_id": 0})
        .sort("created_at", -1)
        .to_list(200)
    )
    await ensure_chat_rooms_for_community(community_id, community_doc["name"], subyards)
    chat_rooms = (
        await chat_rooms_collection.find({"community_id": community_id}, {"_id": 0})
        .sort("created_at", 1)
        .to_list(200)
    )
    return {
...
async def list_subyards(current_user: dict[str, Any] = Depends(get_current_user)):
    subyards = (
        await subyards_collection.find(
...
async def get_subyard(
    subyard_id: str, current_user: dict[str, Any] = Depends(get_current_user)
):
    ...
    announcements = (
        await announcements_collection.find(
            {
                "community_id": community_id,
                "$or": [{"scope": "courtyard"}, {"subyard_id": subyard_id}],
            },
            {"_id": 0},
        )
        .sort("created_at", -1)
    ...
    chat_room = await chat_rooms_collection.find_one(
        {"community_id": community_id, "subyard_id": subyard_id}, {"_id": 0}
    )
    kinships = (
        await kinships_collection.find({"community_id": community_id}, {"_id": 0})
        .sort("created_at", -1)
...
async def create_subyard(
    payload: SubyardCreateRequest,
    current_user: dict[str, Any] = Depends(get_current_user),
):
    ensure_minimum_role(current_user, "organizer")
    ...
    subyard_doc = {
        ...
        "assigned_tools": sorted(
            {tool for role in role_focus for tool in ROLE_TOOLING.get(role, [])}
        ),
    ...
    await ensure_chat_rooms_for_community(
        ...
        (await get_community_for_user(current_user))["name"],
        [subyard_doc],
    )
    return subyard_doc
...
async def update_subyard(
    ...
    role_focus = (
        [role.strip().lower() for role in payload.role_focus if role.strip()]
        if hasattr(payload, "role_focus") and payload.role_focus
        else existing.get("role_focus", [])
    )
    updates = {
...
async def list_kinship(current_user: dict[str, Any] = Depends(get_current_user)):
    relationships = (
        await kinships_collection.find(
            {"community_id": current_user["community_id"]}, {"_id": 0}
        )
        .sort("created_at", -1)
        .to_list(500)
...
async def kinship_graph(current_user: dict[str, Any] = Depends(get_current_user)):
    ...
    for rel in relationships:
        ...
        target = resolve(
            rel.get("related_to_name", ""), rel.get("related_to_user_id", "")
        )
        links.append(
            {
                "source": source,
...
async def kinship_person(
    ...
    if not person:
        raise HTTPException(
            ...
            detail="Person not found in this community.",
        )
    ...
    memories = (
        await memories_collection.find(
            await visible_memory_query_for_user(current_user, created_by=user_id),
...
async def delete_kinship(
    kinship_id: str, current_user: dict[str, Any] = Depends(get_current_user)
):
    result = await kinships_collection.delete_one(
        {"id": kinship_id, "community_id": current_user["community_id"]}
    )
    if result.deleted_count == 0:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Kinship relationship not found.",
        )
    return {"ok": True}
...
async def create_invite(
    ...
    existing_member = await users_collection.find_one(
        {"email": invite_email, "community_id": current_user["community_id"]},
        {"_id": 0},
    )
    if existing_member:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="That person is already a member of this community.",
        )
...
async def list_my_communities(current_user: dict[str, Any] = Depends(get_current_user)):
    ...
    for c in communities:
        c["member_count"] = await users_collection.count_documents(
            {"community_id": c["id"]}
        )
        c["is_active"] = c["id"] == current_user["community_id"]
    return {
        "communities": communities,
        "active_community_id": current_user["community_id"],
    }
...
async def switch_community(
    ...
    if not target_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="community_id is required."
        )
    ...
    await users_collection.update_one(
        {"id": current_user["id"]}, {"$set": {"community_id": target_id}}
    )
...
async def join_community_with_invite(
    body: dict, current_user: dict[str, Any] = Depends(get_current_user)
):
    ...
    if not invite_code:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="invite_code is required."
        )

    invite_doc = await invites_collection.find_one(
        {"code": invite_code, "status": "pending"}, {"_id": 0}
    )
    if not invite_doc:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
