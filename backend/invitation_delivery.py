...
async def apply_first_send_delivery_event(
    *,
    ...
    Delivered events stamp ``delivered_at``/``delivery_verified_at`` exactly once
    on the mapped invitation. Failure events never downgrade a delivered target.
    Unknown provider references are ignored (they may belong to another surface).
    """
